...
import com.cocos.admob.proto.UMPCompleteNTF;
...
public final class AdServiceHub {
    ...
    private boolean umpCompleted = false;
    private boolean umpResult = true;

    public void init(CocosActivity activity) {
        Log.d(TAG, String.format("onCreate {0}", initialized));
        this.activity = activity;
        initialized = false;
        Log.d(TAG, "ADMOB_CHILD_DIRECTED_TREATMENT: " + BuildConfig.ADMOB_CHILD_DIRECTED_TREATMENT);
        if (BuildConfig.ADMOB_CHILD_DIRECTED_TREATMENT) {
        ...
        // Android 不需要 UMP，直接标记完成并立刻发送事件
        umpResult = true;
        umpCompleted = true;

        //不能在这里发送消息，会导致so加载顺序问题，直接崩溃
        // sendUMPCompleteEvent();
    }
    
    private void sendUMPCompleteEvent() {
        UMPCompleteNTF ntf = new UMPCompleteNTF("", "not_required", umpResult);
        bridge.sendToScript(UMPCompleteNTF.class.getSimpleName(), ntf);
    }
